...
No speedup, throughput, or acceptance rate is claimed. Milestones 1 to 6 of 8 are
complete. Greedy and sampled speculation and a measured cost controller all run
...
What has **not** happened is a benchmark: no locked cohort, no held-out split,
no confidence intervals. `RESULTS.md` does not exist and will be
...
- **The controller has only been run in-sample.** Its calibration prompts are
  its evaluation prompts, so it cannot yet be said to generalize.
