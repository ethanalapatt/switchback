...
per-dataset breakdown and provenance in **[RESULTS.md](RESULTS.md)**; the
natural-stop cohort is reported separately in
[`artifacts/runs/natural/report.md`](artifacts/runs/natural/report.md).

![A real trace: four draft candidates accepted, then a block where all four are discarded and the target commits its own token, with the KV cache length changing at each step](docs/trace.gif)

*An actual saved trace, not an illustration — generated from
[`artifacts/traces/greedy_g4.jsonl`](artifacts/traces/greedy_g4.jsonl) by
...
| Natural-stop cohort and no-bypass ablation | Complete, validated, rendered |
...
A full post-mortem — why the controller lost, the three measurement bugs that
...
- **The sampled and context-stress cohorts have not run.** Context stress is
  ...
  vary, which is the only condition under which an adaptive policy can beat a
  well-chosen fixed one.
