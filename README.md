...
No speedup, throughput, or acceptance rate is claimed. Milestones 1 to 4 of 8 are
complete. Fixed greedy speculation runs on Qwen3-4B with a Qwen3-0.6B draft and
produces token-for-token identical output to target-only decoding at draft
lengths 1, 2, 4 and 8. What has **not** happened is a benchmark: no cohort, no
randomized engine order, no confidence intervals. `RESULTS.md` does not exist and will be
...
| Sampled speculation and traces | Not started (M5) |
...
- **No benchmark has run.** Milestones 5–8 are unimplemented: sampled
  speculation, the cost controller, the locked cohort, and the report. The
...
- **Greedy only.** `decode_speculative_greedy` refuses a sampled configuration
  rather than silently approximating one.
