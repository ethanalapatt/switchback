...
No speedup, throughput, or acceptance rate is claimed. Milestone 1 of 8 is
complete: the environment, the pinned model pair, and the two reference
baselines run on real hardware. The Switchback decoder itself is not implemented
...
See [PROGRESS.md](PROGRESS.md) for milestone status and
...
| Sampling oracle, rejection sampler | Not started (M2) |
...
- **The engine does not exist yet.** Milestones 2–8 are unimplemented. Anything
  about speculative decoding performance here is a plan, not a finding.
