...
No speedup, throughput, or acceptance rate is claimed. Milestones 1 and 2 of 8 are
complete: the environment, the pinned model pair and the two reference baselines
run on real hardware, and the sampler is verified exactly against an independent
oracle. The cached engine that would connect them does not exist yet, so there
is nothing to compare. `RESULTS.md` does not exist and will be
...
| Cached target-only engine, KV cache ledger | Not started (M3) |
...
- **The engine does not exist yet.** Milestones 3–8 are unimplemented. The
  sampler is model-free and is driven by row callbacks; nothing connects it to a
  KV cache or to real logits. Anything about speculative decoding performance
  here is a plan, not a finding.
